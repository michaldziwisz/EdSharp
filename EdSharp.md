...
The Recent Files list (Alt+R) and the Favorites list (Alt+L) each offer extra keys on the highlighted file: RightArrow opens it with another program, LeftArrow reads its full path, Control+Enter shows it in Explorer, Control+C copies its path, Delete removes the entry from the list, and Shift+Delete deletes the file from disk after confirming.  The list itself announces only "Press F1 for keys", so a screen reader gets to the file name immediately; press F1 for the full list of these keys, or Shift+F1 to hear them spoken while the list has focus.
...
Use the HTML Format command, Control+H, to convert the current document to HTML in a new window.  The document is treated as Markdown source and rendered to a complete HTML page by the bundled Pandoc, so Markdown headings, lists, links, tables, and emphasis become their HTML equivalents.  Plain text with no Markdown markup converts cleanly as well.
...
Use the Elevate Version command, F11, to update EdSharp to the latest version published on its [GitHub releases page](<https://github.com/JamalMazrui/EdSharp/releases/latest>).  EdSharp checks that page and, if a newer release is available, offers to download and run its installer.
### Hotkey Summary
...
The latest version of EdSharp is available on its [GitHub releases page](<https://github.com/JamalMazrui/EdSharp/releases/latest>).
